...
const students = data.filter(student => student.assistant != true); //console.log(students)

const assistant = data.filter(person => person.assistant === true); //console.log(assistant)
// const filteredStudents = students.map(a => {
// 	console.log(a)
// 	const data3 = students.filter(b => {
// 		console.log(b)
...
let categories = data.reduce((r, {
  ...
  temp.member.push(object);
  return r;
}, []);
console.log(categories);
categories.forEach(element => {
  const cards = document.createElement("DIV");
  const teamLeader = document.createElement("H1");
  cards.className = "groups";
  cards.id = element.group;
  cards.style.backgroundColor = element.group;
  document.querySelector(".main").appendChild(cards);
  cards.appendChild(teamLeader);
  assistant.map(person => {
    if (person.group === cards.id) {
      teamLeader.innerHTML = person.name.toUpperCase();
    }
  });
});
...
allGroups.forEach(team => {
  students.map(stu => {
    if (stu.group === team.id) {
      const li = document.createElement("LI");
      li.innerHTML = stu.name;
      team.appendChild(li);
    }
  });
});
...
},{}]},{},["../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","assets/script/main.js"], null)
